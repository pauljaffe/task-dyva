...
raw_data_dirs = [
    os.path.join(this_dir, "test_data", rdd) for rdd in rel_data_dirs
]
raw_data_fns = ["user1365.pickle", "user1089.pickle", "user2139.pickle"]
...
@pytest.mark.parametrize(
    "resamp, sm, noise_sd, rt_method, out_method, out_thresh", test_conditions
)
def test_ebb_flow_dataset(
    resamp,
    sm,
    noise_sd,
    rt_method,
    ...
    raw_data_fns=raw_data_fns,
    test_dirs=test_dirs,
):
    discrete_check_fields = [
        "onset",
        "offset",
        "point_dir",
        "mv_dir",
    ...
    for rdd, rfn, td in zip(raw_data_dirs, raw_data_fns, test_dirs):
        ...
        for dataset, pre, split in zip(split_datasets, pre_datasets, splits):
            ...
            rates = dataset.xu[:, :, :4]
            binned_rts = bin_response_times(
                dataset.discrete["urt_ms"], dataset.params["step_size"]
            )
