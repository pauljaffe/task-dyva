...
def test_experiment(
    ...
    delete_local,
    orig_training_epochs,
    resume_training_epochs,
    orig_true_epochs,
    resume_true_epochs,
    patience,
    raw_data_dir=raw_data_dir,
    raw_data_fn=raw_data_fn,
    test_dir=test_dir,
):
    # Check that logging, resuming training, and early stopping
    # work correctly.

    tparams = {"start_times": [5000], "upscale_mult": 1, "duration": 5000}
    expt_kwargs = {
        "mode": "full",
        ...
        "val": tparams,
        "test": tparams,
        "num_epochs": orig_training_epochs,
        "keep_every": 1,
        "stop_patience": patience,
        "stop_min_epoch": 0,
        "stop_delta": float("inf"),
    ...
    tester = SetUpTests(
        test_dir, raw_data_dir, raw_data_fn, **resume_expt_kwargs
    )
    expt = tester.make_experiment()
    expt.run_training()
