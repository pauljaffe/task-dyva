...
class PlotRTs(EbbFlowStats):
    ...
    def _format_by_congruency(self):
        ...
        trial_types = ["Congruent", "Incongruent", "Congruent", "Incongruent"]
        m_or_u = ["user", "user", "model", "model"]
        return self._format_as_df(plot_dists, m_or_u, trial_types)
...
class BarPlot:
    ...
    def plot_grouped_bar(self, x, y, hue, error_type, ax, **kwargs):
        ...
        colors = [(0.2363, 0.3986, 0.5104, 1.0), (0.2719, 0.6549, 0.4705, 1.0)]
    ...
    def plot_bar(self, keys, error_type, ax, **kwargs):
        ...
        colors = sns.color_palette(
            palette=self.palette, n_colors=len(plot_data)
        )
        ...
        for di, d in enumerate(plot_data):
            ...
            if show_all:
                sns.swarmplot(y=d, orient="v", color="k", size=2, ax=ax)
        ax = self._adjust_bar(np.arange(len(plot_data)), ax, **kwargs)
        return ax

    def alt_plot_bar(
        self, group_var, group_vals, y_var, error_type, ax, **kwargs
    ):
        assert error_type in self.supported_error, (
            "error_type must be one of the following: "
            f"{self.supported_error}"
        ...
        width = kwargs.get("width", 0.75)
        plot_data = [
            self.df.query(f"{group_var} == @gv")[y_var].values
...
class PlotModelLatents:
    ...
    def __init__(
        ...
        self.t_off_ind = (
            self.n_pre + 1 + np.round(post_on_dur / self.step).astype("int")
        )
        if plot_pre_onset:
            self.t_on_ind = 0
        else:
    ...
    def plot_stay_switch(self, ax, params, elev=30, azim=60):
        ...
        plot_kwargs = {
            ...
            "line_styles": styles,
            "line_width": 0.5,
            "plot_series_onset": True,
            "plot_series_rt": True,
            "plot_times": np.array([100]),
        }
        ax = self.plot_3d(
            series, labels, ax, elev=elev, azim=azim, **plot_kwargs
        )
        return ax
    ...
    def plot_main_conditions(
        self, ax, elev=30, azim=60, plot_task_centroid=False, **kwargs
    ):
    ...
    def _get_main_series(self, stim_cue_vals):
        ...
        for this_stim_cue in stim_cue_vals:
            ...
            if this_cue == 0:  # moving task
                this_filters = {"mv_dir": this_stim, "task_cue": this_cue}
            else:  # pointing task
                this_filters = {"point_dir": this_stim, "task_cue": this_cue}
    ...
    def _plot_3d_line(self, ax, series_inds, color, style, label, width):
        x = np.mean(
            self.latents[self.t_on_ind : self.t_off_ind, series_inds, 0], 1
        )
        y = np.mean(
            self.latents[self.t_on_ind : self.t_off_ind, series_inds, 1], 1
        )
        z = np.mean(
            self.latents[self.t_on_ind : self.t_off_ind, series_inds, 2], 1
        )
        ax.plot(
            x, y, z, color=color, label=label, linestyle=style, linewidth=width
        )
        return ax
