...
class _EncoderHZ2W(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.hz2w_shared = nn.Sequential(
            nn.Linear(self.encoder_rnn_hidden_dim + self.latent_dim, 
...
class _EncoderXU2H(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.encoder_mlp = nn.Sequential(
            nn.Linear(self.input_dim + self.u_dim, 
                      self.encoder_mlp_hidden_dim),
            nn.ReLU(),
            nn.Linear(self.encoder_mlp_hidden_dim, 
                      self.encoder_rnn_input_dim))

        encoder_rnn = nn.LSTM(
            self.encoder_rnn_input_dim, self.encoder_rnn_hidden_dim,
...
class LLDVAE(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.pw = getattr(dist, params['model_params'].get(
            'prior_dist', 'Normal'))
        self.px_w = getattr(dist, params['model_params'].get(
        ...
        self._pw_params = nn.ParameterList([
            nn.Parameter(torch.zeros(1, 1, self.latent_dim, 
                                     device=self.device), 
                         requires_grad=False), 
            nn.Parameter(torch.ones(1, 1, self.latent_dim, 
                                    device=self.device), 
                         **prior_grad)])
        ...
        B_mats = torch.empty(num_mats, self.latent_dim, self.u_dim,
                             device=self.device)
        self.B_mats = Parameter(nn.init.normal_(B_mats))
        self.A_mats = Parameter(self.dyn_mat_mult * _init_dynamics_mats(
            self.latent_dim, num_mats, self.rand_seed, self.dyn_init))
        self.C_mats = Parameter(self.dyn_mat_mult * _init_dynamics_mats(
            self.latent_dim, num_mats, self.rand_seed, self.dyn_init))
    ...
    def pw_params(self):
        ...
        pw_vars = F.softmax(self._pw_params[1], 
    ...
    def _init_w_z(self, batch_size):
        ...
        w0_vars = (F.softmax(
            self.w0_vars.unsqueeze(0).expand(
                1, batch_size, self.w_dim), dim=-1) 
            * self.w0_vars.size(-1) + Constants.eta)
    ...
    def _setup_propagate(self, xu, generate_mode, clamp):
        ...
        if generate_mode:
            ...
            w0 = self.pw(*self.pw_params).expand(
                (seq_length, n_data, self.w_dim)).rsample()
            if clamp:
                w0 = torch.zeros(seq_length, n_data, 
                                 self.w_dim, device=self.device)
