...
class EbbFlowDataset(Dataset):
    ...
    def get_processed_sample(self, idx):
        ...
        return EbbFlowGameData.processed_format(
            discrete, continuous, self.params, game_id
        )
    ...
    def process(self):
        ...
        if (self.resampling_type in self.needs_data_augmentation) or (
            self.params["smoothing_type"] in self.needs_data_augmentation
        ):
        ...
        if self.supplied_pre is not None:
            supplied_pre = [
                t(p)
                for t, p in zip(self.supplied_pre, self.supplied_pre_params)
            ]
    ...
    def _get_preprocessed_games(self, for_resampling=False):
        ...
        for start_time, game_ind, _ in product(
            start_times,
            range(len(self.preprocessed["urt_ms"])),
            range(upscale_mult),
        ):
            preprocessed_game = {
                key: self.preprocessed[key][game_ind]
    ...
    def _get_resampling_sm_info(self, data):
        ...
        sm_params = {
            ...
            "smoothing_type": self.params.get("smoothing_type", "gaussian"),
        ...
        for ttype in range(4):
            ...
            for d in data:
                ...
                this_correct.extend(
                    d._get_field_by_trial_type(ttype, "ucorrect")
                )
    ...
    def _collate(self, data_list):
        ...
        xu_split = [
            torch.cat(
                [
                    torch.tensor(d.continuous[key]).unsqueeze(1)
                    for d in data_list
                ],
                1,
            )
        ...
        self.discrete = {
            key: [d.discrete[key] for d in data_list] for key in disc_keys
        }
...
class EbbFlowStats(EbbFlowDataset):
    ...
    **kwargs (optional): Extra options to be supplied for calls to
    ...
    def __init__(self, output_rates, dataset, latents=None, n_pcs=3, **kwargs):
        ...
        for d in range(self.rates.shape[1]):
            ...
            if self.latents is not None:
                ...
                win_vars = this_game.get_extra_stats(
                    output_rates=this_rates,
                    latents=this_latents,
                    pca_latents=this_pca_latents,
                    **self.trial_data_kwargs,
                )
            else:
                win_vars = this_game.get_extra_stats(
    ...
    def switch_cost(self):
        ...
        stats = {}
        u_stay_inds = self.select(
            **{"is_switch": 0, "ucorrect": 1, "u_prev_correct": 1}
        )
        m_stay_inds = self.select(
            **{"is_switch": 0, "mcorrect": 1, "m_prev_correct": 1}
        )
        u_switch_inds = self.select(
            **{"is_switch": 1, "ucorrect": 1, "u_prev_correct": 1}
        )
        m_switch_inds = self.select(
            **{"is_switch": 1, "mcorrect": 1, "m_prev_correct": 1}
        )
        # response times
        ...
        u_acc_switch_inds = self.select(
            **{"is_switch": 1, "u_prev_correct": 1}
        )
        m_acc_switch_inds = self.select(
            **{"is_switch": 1, "m_prev_correct": 1}
        )
        ...
        stats["u_switch_cost_estop"] = (
            u_switch_rts_estop.mean() - u_stay_rts_estop.mean()
        )
        stats["m_switch_cost_estop"] = (
            m_switch_rts_estop.mean() - m_stay_rts_estop.mean()
        )
        return stats
    ...
    def congruency_effect(self):
        ...
        stats["u_con_effect_estop"] = (
            u_incon_rts_estop.mean() - u_con_rts_estop.mean()
        )
        stats["m_con_effect_estop"] = (
            m_incon_rts_estop.mean() - m_con_rts_estop.mean()
        )
        return stats
...
class EbbFlowGameData:
    ...
    def __init__(
        ...
        self.num_steps_short_win = int(np.rint(params["duration"] / self.step))
        self.num_steps_long_win = int(
            np.rint(
                (params["duration"] + self.extra_time_for_smooth) / self.step
            )
        )
    ...
    @classmethod
    def preprocessed_format(
        cls, preprocessed, params, start_time, resampling_info=None
    ):
    ...
    def _initialize_arrays(cls, params):
        ...
        num_samples = int(
            np.rint(
                (params["duration"] + cls.extra_time_for_smooth)
                / params["step_size"]
            )
        )
        continuous = {
            key: np.zeros((num_samples, d))
            for key, d in zip(cls.continuous_fields[1:], cls.dims[1:])
        }
    ...
    def standard_prep(self):
        ...
        if self.resampling_info is None:
            ...
            prev_cue = self._map_str_to_num(
                random.sample(self.task_labels, 1)[0], "task_cue"
            )
    ...
    def _get_trial_info_preprocessed(self, trial_ind, abs_offset_ms, prev_cue):
    ...
    def _resample_trial(self, trial_info, prev_cue):
        ...
        new_mv_dir = self._map_str_to_num(
            random.sample(self.direction_labels, 1)[0], "mv_dir"
        )
        if con_ind in [0, 2]:
            # Congruent trial
        ...
        new_data = {
            "urt_ms": new_rt_ms[0][0],
            "task_cue": new_cue,
            "mv_dir": new_mv_dir,
            "point_dir": new_pt_dir,
            "urespdir": new_uresp,
        }
    ...
    def _get_resampler_trial_type(self, trial_info, prev_cue):
        is_congruent = self._is_congruent(
            trial_info["mv_dir"], trial_info["point_dir"]
        )
        if np.isnan(prev_cue):
    ...
    def _update_continuous(self, trial_info):
        for key in ["mv_dir", "point_dir", "task_cue"]:
            self.continuous[key][
                trial_info["onset"] : trial_info["offset"], trial_info[key]
            ] = 1

        abs_rt = trial_info["onset"] + trial_info["urt_samples"]
        self.continuous["urespdir"][
            abs_rt, trial_info["urespdir"], trial_info["trial_type"]
        ] = 1

    def get_extra_stats(
        self, output_rates=None, latents=None, pca_latents=None, **kwargs
    ):
        ...
        for n in range(n_trials):
            ...
            tr.update(
                {key: self.discrete[key][n] for key in self.discrete_fields}
            )

            # Stimulus info
            tr["correct_dir"] = self._get_correct_dir(
                tr["task_cue"], tr["mv_dir"], tr["point_dir"]
            )
            tr["is_congruent"] = self._is_congruent(
                tr["mv_dir"], tr["point_dir"]
            )
            if n != 0:
                tr["is_switch"] = self._is_switch(
                    tr["task_cue"], tr["prev_task_cue"]
                )

            # User response info
            tr["ucorrect"] = self._is_correct(
                tr["urespdir"], tr["correct_dir"]
            )
            ...
            if output_rates is not None:
                (
                    ...
                    tr["mrt_abs"],
                    tr["mrespdir"],
                ) = self._get_model_rt(tr["onset"], tr["offset"], output_rates)
                tr["mcorrect"] = self._is_correct(
                    tr["mrespdir"], tr["correct_dir"]
                )
    ...
    def _get_windowed(self, onset, data, win_length, n_pre, n_post):
        ...
        if windowed.shape[0] < win_length:  # zero pad
            windowed = np.append(
                windowed, np.zeros((win_length - windowed.shape[0], dim)), 0
            )
        ...
        for d in range(4):
            ...
            if rates is not None:
                ...
                this_mrts = [
                    rt
                    for rt, rdir in zip(
                        self.discrete["mrt_abs"], self.discrete["mrespdir"]
                    )
                    if rdir == d
                ]
                ...
                axes[d + 10].set_ylabel(
                    self.direction_labels[d], fontsize=textsize
                )
    ...
    def _get_model_rt(self, onset, offset, rates):
        ...
        max_ind = np.unravel_index(
            np.argmax(win_rates, axis=None), win_rates.shape
        )
        mrespdir = max_ind[1]
    ...
    def _get_field_by_trial_type(self, trial_type, key):
        inds = [
            i
            for i, val in enumerate(self.discrete["trial_type"])
            if val == trial_type
        ]
