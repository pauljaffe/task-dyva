        ...
        if os.path.exists(self.fp_path) and self.load_saved:
            ...
            for c in self.stimulus_combos:
                ...
                c_stimuli = self._make_fp_stimuli(
                    N, T, pt=pt, mv=mv, cue=cue
                ).to(self.device)
                # Generate responses
                _, _, _, z_out, _, _ = self.expt.model.forward(
                    c_stimuli, generate_mode=True, clamp=True, z0_supplied=z0
                )
...
class LatentsLDA:
    ...
    def _split_trials_by_task(self):
        ...
        self.mv_z = np.squeeze(
            self.expt_stats.windowed["latents"][self.t_ind, mv_inds, :]
        )
        self.pt_z = np.squeeze(
            self.expt_stats.windowed["latents"][self.t_ind, pt_inds, :]
        )
        self.mv_N = len(mv_inds)
        self.pt_N = len(pt_inds)
        self.task_y = np.hstack((np.zeros(self.mv_N), np.ones(self.pt_N)))
        ...
        if os.path.exists(self.save_path) and self.load_saved:
            ...
            for cue in cues:
                for d in directions:
                    ...
                    this_x1 = np.squeeze(
                        self.expt_stats.windowed["latents"][
                            self.t_ind, x1_inds, :
                        ]
                    )
                    this_x2 = np.squeeze(
                    ...
                    this_y = np.hstack(
                        (np.zeros(len(x1_inds)), np.ones(len(x2_inds)))
                    )

                    # True within task error
                    true_within_errors.append(
                        self._get_lda_error(this_x, this_y)
                    )
    ...
    def _get_shuffle_error(self, x1, x2):
        ...
        for n in range(self.n_shuffle):
            ...
            this_y = np.hstack(
                (np.zeros(len(this_x1_inds)), np.ones(len(this_x2_inds)))
            )
...
class LatentSeparation:
    ...
    def analyze(self):
        ...
        centroid_dist = np.linalg.norm(
            self.mv_centroid - self.pt_centroid, ord=2
        )
