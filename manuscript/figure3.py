...
class Figure3:
        ...
        for expt_str, uid, sc, exg, early, opt in zip(
            ...
            if uid == self.example_user:
                fp_path = os.path.join(
                    self.model_dir, expt_str, self.analysis_dir, self.fp_fn
                )
            ...
            fp_summary_path = os.path.join(
                self.model_dir, expt_str, self.analysis_dir, self.fp_summary_fn
            )
    ...
    def _make_panel_A(self, ax):
        ...
        _ = plotter.plot_main_conditions(
            ax, elev=elev, azim=azim, plot_task_centroid=False, **kwargs
        )
    ...
    def _make_panel_B(self, ax):
        ...
        for pc_ind, pc in enumerate(x):
            print(
                f"PC {pc} mean +/- s.e.m. cumulative explained var.: "
    ...
    def _make_panel_C(self, ax):
        ...
        w_task, p_task = wilcoxon(
            df["bw_error"].values,
            y=df["bw_shuffle_error"].values,
            mode="approx",
        )
        w_direction, p_direction = wilcoxon(
            df["within_error"].values,
            y=df["within_shuffle_error"].values,
            mode="approx",
        )
        print(
            f"Within vs. between task, signed-rank test: w_stat = {w_task}, "
            f"p = {p_task}, N = {len(df)}"
        )
        print(
            "Within task, same vs. different relevant direction, "
            f"signed-rank: w_stat = {w_direction}, p = {p_direction}, "
            f"N = {len(df)}"
        )
        for key in keys:
            print(
                f"{key} mean +/- s.e.m. misclassification rate: "
    ...
    def _make_panel_D(self, ax, df):
        ...
        w_task, p_task = wilcoxon(
            df["within_task"].values,
            y=df["between_task"].values,
            mode="approx",
        )
        w_direction, p_direction = wilcoxon(
            df["same_response"].values,
