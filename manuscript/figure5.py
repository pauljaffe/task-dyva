...
class Figure5:
    ...
    def __init__(self, model_dir, save_dir, metadata, rand_seed, n_boot):
        ...
        self.sc_minus_stats = {
            n: copy.deepcopy(stat_dict) for n in self.noise_labels
        }
        self.sc_plus_stats = copy.deepcopy(self.sc_minus_stats)
        diff_dict = {"acc_diff": [], "u_con_effect": []}
        self.acc_diff_stats = {
            n: copy.deepcopy(diff_dict) for n in self.noise_labels
        }
    ...
    def _make_panel_A(self, ax):
        ...
        for n_label, n_sd in zip(self.noise_labels, self.noise_sds):
            ...
            sc_minus_vals = np.array(
                self.sc_minus_stats[n_label]["m_accuracy"]
            )
            sc_plus_means = np.append(sc_plus_means, np.mean(sc_plus_vals))
            sc_minus_means = np.append(sc_minus_means, np.mean(sc_minus_vals))
            sc_plus_errors = np.append(
                sc_plus_errors, np.std(sc_plus_vals) / np.sqrt(N)
            )
            sc_minus_errors = np.append(
                sc_minus_errors, np.std(sc_minus_vals) / np.sqrt(N)
            )
        ...
        ax.fill_between(
            self.noise_sds,
            sc_minus_means - sc_minus_errors,
            sc_minus_means + sc_minus_errors,
            alpha=0.2,
            facecolor=cmap(0.7),
            label=None,
    ...
    def _make_panel_B(self, ax):
        ...
        sns.heatmap(
            delta_np,
            center=0,
            cmap=self.heatmap_palette,
            ax=ax,
            vmin=-0.4,
            vmax=0.4,
            cbar_kws={"ticks": [-0.25, 0, 0.25]},
        )
    ...
    def _make_panel_C(self, ax):
        ...
        ylab = "Model robustness to noise"
...
def plot_scatter2(
    ...
    # Plot best fit line
    plot_x = np.array([min(d1) - line_ext, max(d1) + line_ext])
    ...
    r, p, ci_lo, ci_hi = pearson_bootstrap(
        d1, d2, rng, n_boot=n_boot, alpha=alpha
    )
    p_str = "{:0.2e}".format(p)
