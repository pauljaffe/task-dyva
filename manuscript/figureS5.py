...
class FigureS5:
    ...
    def _run_preprocessing(self):
        for expt_str, uid, exg, earl, sc, opt in zip(
            ...
            model_hist = np.histogram(model_rts, bins=bin_edges, density=True)[
                0
            ]
    ...
    def _plot_figure_get_stats(self):
        ...
        exgauss_sem = np.std(self.exgauss_klds) / np.sqrt(
            len(self.exgauss_klds)
        )
