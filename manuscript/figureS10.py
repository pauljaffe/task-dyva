...
class FigureS10:
    ...
    def _make_error_panel(self, ax, key, title):
        ...
        for i, n in enumerate(self.noise_labels):
            ...
            m_sc_plus, lo_sc_plus, hi_sc_plus = pj_bootstrap(
                sc_plus_vals,
                self.rng,
                self.n_boot,
                self.alpha,
            ...
            m_sc_minus, lo_sc_minus, hi_sc_minus = pj_bootstrap(
                sc_minus_vals,
                self.rng,
                self.n_boot,
                self.alpha,
    ...
    def _make_panel_E(self, ax):
        ...
        cis = np.zeros((2, N))
        print(
            "Stats for diff. in model accuracy vs. participant congruency effect"
        )
        print("----------------------------------------")
        for ind, pn, pnlab in zip(
            range(N), self.noise_sds[1:], self.noise_labels[1:]
        ):
            ...
            r, p, ci_lo, ci_hi = pearson_bootstrap(
                x, y, self.rng, n_boot=self.n_boot, alpha=self.alpha
            )
            rvals.append(r)
            cis[0, ind] = r - ci_lo
            cis[1, ind] = ci_hi - r
        ...
        ax.errorbar(
            self.noise_sds[1:], rvals, yerr=cis, linestyle="-", linewidth=0.5
        )
        ax.set_xlabel("Noise SD")
