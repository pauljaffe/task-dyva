...
class FigureS8:
    ...
    def _run_preprocessing(self):
        for expt_str, ab, uid, sc, exg, early, opt in zip(
            ...
            summary_path = os.path.join(
                self.model_dir, expt_str, self.analysis_dir, self.summary_fn
            )
            ...
            # Panel e (age)
            self.age_data.append(
                [ab, this_summary["01"]["normed_centroid_dist"]]
            )
        ...
        self.age_df = pd.DataFrame(
            self.age_data, columns=["age_bin", "normed_centroid_dist"]
        )
    ...
    def _make_panel_D(self, ax):
        ...
        for ind, n, sd in zip(
            range(len(self.noise_labels)), self.noise_labels, self.noise_sds
        ):
        ...
        ax.errorbar(
            self.noise_sds, r_vals, yerr=cis, linestyle="-", linewidth=0.5
        )
    ...
    def _make_panel_E(self, ax):
        ...
        group_var = "age_bin"
        anova_data = [
            self.age_df.query(f"{group_var} == @gv")[
        ...
        tukey = pairwise_tukeyhsd(
            endog=self.age_df["normed_centroid_dist"],
