...
from task_dyva.model_analysis import (
    FixedPointFinder,
    LatentsLDA,
    LatentSeparation,
)
...
class Preprocess:
    ...
    def run_preprocessing(self):
        for expt_str, model_type, exg, this_early, opt in zip(
            self.expts, self.sc_status, self.exgauss, self.early, self.optimal
        ):
            print(f"Preprocessing experiment {expt_str}")
            this_model_dir = os.path.join(self.model_dir, expt_str)
            # Get model outputs
            self._get_outputs_wrapper(
                this_model_dir, expt_str, exg, this_early, opt
            )
            # Get model and behavior summary metrics
            # (including distance b/w task centroids)
            self._get_summary(this_model_dir, model_type, exg, this_early, opt)
            # Find stable fixed points
            self._fp_wrapper(
                this_model_dir, expt_str, model_type, exg, this_early, opt
            )
            # LDA analysis
            self._lda_wrapper(this_model_dir, model_type, exg, this_early, opt)
            # Clean up
            self._clean_up(this_model_dir, exg, this_early, opt)

    def _get_outputs_wrapper(
        self, model_dir, expt_str, exgauss, early, optimal
    ):
        if exgauss == "exgauss+" or early or optimal:
    ...
    def _get_model_outputs(
        self, model_dir, expt_str, noise_key, noise_sd, analyze_latents, early
    ):
        save_str = f"{self.outputs_save_str}_{noise_key}SD.pkl"
        noise_params = {"noise_type": "indep", "noise_sd": noise_sd}
        if early:
            expt_kwargs = {
                "logger_type": None,
                "test": noise_params,
    ...
    def _get_summary(self, model_dir, model_type, exgauss, early, optimal):
        if exgauss == "exgauss+" or early or optimal:
            noise_keys = [self.primary_noise_key]
            noise_sds = [self.primary_noise_sd]
        else:
        ...
        # Save
        save_path = os.path.join(model_dir, self.analysis_dir, self.summary_fn)
    ...
    def _fp_wrapper(
        self, model_dir, expt_str, model_type, exgauss, early, optimal
    ):
        if exgauss == "exgauss+" or early or optimal:
            return

        fp_path = os.path.join(model_dir, self.analysis_dir, self.fp_fn)
        fp_summary_path = os.path.join(
            model_dir, self.analysis_dir, self.fp_summary_fn
        )
        if model_type != "sc-":
            noise_params = {
                "noise_type": "indep",
            ...
            outputs = self._reload_outputs(model_dir, self.primary_noise_key)
            ...
            fpf.get_fixed_point_summary(this_fps)

    def _lda_wrapper(self, model_dir, model_type, exgauss, early, optimal):
        if exgauss == "exgauss+" or optimal:
            return

        lda_path = os.path.join(model_dir, self.analysis_dir, self.lda_fn)
        if model_type != "sc-":
            outputs = self._reload_outputs(model_dir, self.primary_noise_key)
            lda = LatentsLDA(
                outputs,
    ...
    def _reload_outputs(self, model_dir, noise_key):
        ...
        outputs_path = os.path.join(model_dir, self.analysis_dir, outputs_fn)
        with open(outputs_path, "rb") as path:
            outputs = pickle.load(path)
        return outputs

    def _clean_up(self, model_dir, exgauss, early, optimal):
        if exgauss == "exgauss+" or early or optimal:
            return
        ...
        for noise_key in self.all_noise_keys:
            if noise_key != self.primary_noise_key:
                ...
                outputs_path = os.path.join(
                    model_dir, self.analysis_dir, outputs_fn
                )
